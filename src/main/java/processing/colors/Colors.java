 ...
 */
public class Colors {
	public static final int[][] RGBArr = {
		{ 25, 25, 25 },
		{ 21, 21, 21 },
		{ 17, 17, 17 },
		{ 13, 13, 13 },
	...
	public static int HexToHex(int hex)
	{
		int r = hex >>> 16 & 255;
		int g = hex >>> 8 & 255;
		int b = hex >>> 0 & 255;
		...
		{
			double diff = Math.sqrt(
					Math.pow(Math.abs(RGBArr[i][0]) - Math.abs(r), 2) +
					Math.pow(Math.abs(RGBArr[i][1]) - Math.abs(g), 2) +
					Math.pow(Math.abs(RGBArr[i][2]) - Math.abs(b), 2)
				);
		...
		int[] rgb = RGBArr[minIndex];
		
		return (rgb[0] << 24) + (rgb[1] << 16) + (rgb[2] << 8);
	}
	
	public static int[] RGBtoRGB(int r, int g, int b)
	{
		double minDiff = Integer.MAX_VALUE;
		int minIndex = 0;
